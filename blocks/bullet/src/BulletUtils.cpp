...
{ 
	...
	namespace bullet 
	{
		...
		// Create soft box
		btSoftBody * create(btDynamicsWorld * world, btSoftBodyWorldInfo & info, const Vec3f & size, const Vec3f & position, const Quatf & rotation)
		{

			// Define cube
			const btVector3	hull = bullet::toBulletVector3(size) * 0.5f;
			...
			transform.setOrigin(bullet::toBulletVector3(position));
			body->transform(transform);
			body->setTotalMass((size.x * 0.5f) * (size.y * 0.5f) * (size.z * 0.5f) * (float)M_PI * 4.0f / 3.0f, true);
